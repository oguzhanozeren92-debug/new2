...
function selectTodayEvents(events: HomeDecisionEvent[]): HomeDecisionEvent[] {
  ...
  return events.filter(event => event.channels.includes('today') && event.today).filter(event => {
    if (seenGroups.has(event.group)) return false; seenGroups.add(event.group); return true;
  }).slice(0, 2);
}
...
export function useHomeDecisionEngine(input: HomeDecisionEngineInput) {
  ...
  const events = useMemo(() => {
    ...
    const anomalyEvent = buildNdviAnomalyDecision(String(input.homeFieldId ?? ''), input.satelliteAnomaly, activeGrowth, now);

    let merged = baseEvents;
    if (riskEvent) {
      const hasSpatialAlert = Boolean(String(input.fieldSynthesis?.importantArea?.area ?? '').trim());
      merged = hasSpatialAlert ? merged : merged.filter(event => !(event.group === 'pusula' && event.source === 'pusula'));
    }
    // Güçlü negatif anomali varsa aynı NDVI verisinden üretilen basit falling-trend
    // kartını gösterme; anomali olayı daha yüksek kanıt seviyesidir.
  ...
  }, [resolvedInput, input.homeFieldId, input.fieldSynthesis, input.now, input.phenology, input.satelliteAnomaly]);

  const todayDecisions = useMemo(() => {
    const selected = selectTodayEvents(events); const source = selected.length > 0 ? selected : [makeAllClearEvent(input.fieldKey)];
    return source.map(toTodayDecision).filter((item): item is HomeTodayDecision => Boolean(item));
