...
import { PusulaPdfPanel } from '../../pusula-pdf/components/PusulaPdfPanel';
...
function SeasonVarietyEditor({ season }: { season: FieldSeason }) {
  ...
  useEffect(() => {
    setValue(season.varietyName ?? ''); setSavedValue(season.varietyName ?? ''); setMessage('');
  }, [season.id, season.varietyName]);

  const save = async () => {
    if (busy) return;
    const normalized = value.trim();
    if (normalized.length > 120) { setMessage('Çeşit adı 120 karakteri geçemez.'); return; }
    setBusy(true); setMessage('');
    try {
      const { data: { user }, error: userError } = await supabase.auth.getUser();
      if (userError) throw userError; if (!user) throw new Error('Çeşit bilgisini kaydetmek için oturum gerekli.');
      const { error } = await supabase.from('field_seasons').update({ variety_name: normalized || null }).eq('id', season.id).eq('field_id', season.fieldId).eq('user_id', user.id);
      if (error) throw error;
      setSavedValue(normalized); setValue(normalized); setMessage(normalized ? 'Çeşit kaydedildi.' : 'Çeşit bilgisi kaldırıldı.');
      const readiness = await refreshModelReadiness(season.fieldId, 'pcse');
      if (!readiness.ok) console.warn('[season-variety] PCSE readiness refresh failed', readiness.error);
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Çeşit kaydedilemedi.'); }
    finally { setBusy(false); }
  };

  const changed = value.trim() !== savedValue;
  return <div className="tp-field-season-variety">
    <label>Çeşit (varsa)<input value={value} maxLength={120} onChange={(event) => { setValue(event.target.value); setMessage(''); }} placeholder="Örn. Bezostaja, Esperia..." /></label>
    <button type="button" onClick={() => void save()} disabled={busy || !changed}>{busy ? 'Kaydediliyor…' : 'Çeşidi kaydet'}</button>
    {message && <small role="status">{message}</small>}
    <p>Bu çiftçinin gerçek çeşit bilgisidir. Model çeşidi ayrıca doğrulanır; sistem varsayılan model çeşidi seçmez.</p>
  </div>;
}

export default function FieldSeasonSummary({ seasons, activities, loading }: { seasons: FieldSeason[]; activities: FieldActivity[]; loading: boolean; }) {
  const [expanded, setExpanded] = useState(false);
  const now = new Date();
  const today = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
  const summaries = buildFieldSeasonSummaries(seasons, activities, today);
  const fieldId = seasons[0]?.fieldId ? String(seasons[0].fieldId) : '';
  return <>
  <section className="tp-field-season-summary" aria-label="Sezon masraf ve hasat özeti">
  ...
  {fieldId && <PusulaPdfPanel fieldId={fieldId} />}
  </>;
}
