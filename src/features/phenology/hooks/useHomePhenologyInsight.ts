...
import { analyzeNdviAnomaly } from '../../satellite/services/ndviAnomaly.service';
...
export function useHomePhenologyInsight(field: any | null | undefined) {
  ...
  const ndviTrend = useMemo(() => {
    if (!rawTrend || !observationFresh) return null;
    return {
      direction: rawTrend.quality === 'usable' ? rawTrend.direction : 'unknown',
  ...
  const phenologyField = useMemo(() => {
    ...
    return {
      ...
      cropCycle: context?.cropCycle ?? field.cropCycle ?? field.crop_cycle ?? null,
      actualHarvestDate: context?.actualHarvestDate ?? field.actualHarvestDate ?? field.actual_harvest_date ?? null,
      sowingDate: context?.actualPlantingDate ?? field.sowingDate ?? field.sowing_date ?? field.plantingDate ?? field.planting_date ?? null,
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void loadFieldPhenologyContext(field).then(data => {
      if (!cancelled) setContextState({ status: 'ready', data, message: data.needsCropCalendar ? 'Ürün evresi kaynak tabanlı ürün takvimiyle hesaplanacak.' : null });
    }).catch(error => {
      if (cancelled) return;
      if (requestedContextRef.current === fieldKey) requestedContextRef.current = '';
      setContextState({ status: 'error', data: null, message: error instanceof Error ? error.message : 'Fenoloji bağlamı alınamadı.' });
    });
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetchPhenologyClimateShift(field).then(data => {
      if (!cancelled) setClimateShiftState({ status: 'ready', data, message: data.message ?? data.caution });
    }).catch(error => {
      if (cancelled) return;
      if (requestedClimateRef.current === fieldKey) requestedClimateRef.current = '';
      setClimateShiftState({ status: 'error', data: null, message: error instanceof Error ? error.message : 'Fenoloji iklim düzeltmesi alınamadı.' });
    });
    return () => { cancelled = true; };
  }, [fieldKey, field]);
  ...
  useEffect(() => {
    ...
    void load(field, { daysBack: 90 }).catch(error => {
      if (requestedNdviFieldRef.current === key) requestedNdviFieldRef.current = '';
  ...
  useEffect(() => {
    ...
    const signature = `${fieldKey}:${timeSeriesPoints.map(point => `${point.date}:${point.average}`).join('|')}`;
    ...
    let cancelled = false;
    void analyzeNdviAnomaly(timeSeriesPoints).then(data => {
      if (!cancelled) setAnomalyState({ status: 'ready', data, message: data.reason });
    }).catch(error => {
      if (cancelled) return;
      if (anomalySignatureRef.current === signature) anomalySignatureRef.current = '';
      setAnomalyState({ status: 'error', data: null, message: error instanceof Error ? error.message : 'NDVI anomali analizi alınamadı.' });
    });
    return () => { cancelled = true; };
  ...
  return {
    ...
    timeSeriesMessage: latestObservationDate && !observationFresh ? 'Son uydu görüntüsü eski; güncel bitki gelişimi yorumunda kullanılmadı.' : seriesState?.message ?? seriesState?.data?.message ?? null,
    timeSeriesObservationCount: rawTrend?.observationCount ?? 0,
    timeSeriesSpanDays: rawTrend?.spanDays ?? null,
    timeSeriesLatestDate: timeSeriesPoints.map(point => point.date).sort().at(-1) ?? null,
    phenologyDataStatus: phenology?.dataStatus ?? 'insufficient_data',
    hasUsablePhenology: Boolean(phenology && phenology.stage !== 'unknown' && phenology.dataStatus === 'usable'),
