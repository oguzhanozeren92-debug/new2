...
.tp-dual-water-status-head{
  display:flex;
  align-items:flex-start;
  justify-content:space-between;
  gap:12px;
}
.tp-dual-water-status-kicker{
  display:block;
  color:#737373;
  font-size:9px;
  font-weight:800;
...
.tp-dual-water-status h3{
  margin:5px 0 0;
  font-size:16px;
  line-height:1.25;
  color:#171717;
}
.tp-dual-water-status-pill{
  flex:0 0 auto;
  padding:7px 9px;
  border-radius:999px;
  background:#f5f5f5;
...
.tp-dual-water-status-main{
  margin-top:14px;
  padding:13px 14px;
  border-radius:14px;
  background:#f7f7f8;
}
.tp-dual-water-status-main span{
  display:block;
  color:#737373;
  font-size:10px;
}
.tp-dual-water-status-main strong{
  display:block;
  margin-top:4px;
  color:#171717;
  font-size:20px;
}
.tp-dual-water-status-main p,
.tp-dual-water-status-note{
  margin:7px 0 0;
  color:#737373;
  font-size:10.5px;
  line-height:1.45;
}
.tp-dual-water-status ul{
  margin:12px 0 0;
  padding:0;
  list-style:none;
  display:grid;
...
export default function DualKcShadowStatus({
  fieldId,
  irrigationStatus,
}: Props) {
  ...
  const reload = async (runIfMissing = false) => {
    ...
    setError('');
    try {
      let latest = await loadLatestDualKcShadowAudit(id);
      if (!latest && runIfMissing && !rainfed) {
        await runDualKcShadowEvidence(id);
        latest = await loadLatestDualKcShadowAudit(id);
      }
      setAudit(latest);
    } catch (reason) {
  ...
  const missing = [...new Set(audit?.missingInputs ?? [])]
    .map(labelForMissing)
    .slice(0, 5);
  ...
  return (
    <section className="tp-dual-water-status" aria-label="Gelişmiş su modeli durumu">
      ...
      <div className="tp-dual-water-status-head">
        <div>
          <small className="tp-dual-water-status-kicker">SU DENGESİ · MODEL KANITI</small>
          <h3>Su modeli</h3>
        </div>
      ...
      {rainfed ? (
        <p className="tp-dual-water-status-note">
          Tarla susuz olarak kayıtlı. Bu sulama modeli bu tarla için reçete üretmez; yağış ve kuraklık riski ayrı izlenir.
        </p>
        ...
        <div className="tp-dual-water-status-main">
          ...
          <p>
            Belirsizlik aralığı korunur. Bu sonuç henüz otomatik sulama emri veya kesin su miktarı değildir.
          </p>
        </div>
      ) : (
        <p className="tp-dual-water-status-note">
          {loading
            ? 'Gerçek saha verileri ve model kanıtları kontrol ediliyor.'
            : 'Model, eksik saha verisi varken değer uydurmaz. Gerekli bilgiler tamamlandıkça otomatik yeniden kontrol edilir.'}
        </p>
      )}

      {!rainfed && missing.length > 0 ? (
        <ul>
      ...
      {!rainfed ? (
        <p className="tp-dual-water-status-note">
          Bu kart yalnız model kanıtını gösterir. TarlaPusula sulama kararında production otoritesi olarak kullanılmıyor.
        </p>
      ) : null}
    </section>
  );
